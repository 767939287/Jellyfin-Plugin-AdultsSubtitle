...
namespace Jellyfin_Plugin_AdultsSubtitle
{
    public class AdultsSubtitleProvider : ISubtitleProvider
    {
        ...
        public AdultsSubtitleProvider(ILogger logger)
        {
            _logger = logger;
        }
        public async Task<SubtitleResponse> GetSubtitles(string id, CancellationToken cancellationToken)
        {
            ...
            {
                _logger.Info($"start download subtitle {targetSub}");
                ...
                ms.Position = 0;
                _logger.Info($"subtitle {targetSub} download comlete");
        ...
        public async Task<IEnumerable<RemoteSubtitleInfo>> Search(SubtitleSearchRequest request, CancellationToken cancellationToken)
        {
            ...
            _logger.Info($"start search {searchName} {request.Language} subtitle ");
            if (!Api.LanguagesMaps.TryGetValue(request.Language, out var subCatLanguage))
            {
                _logger.Info($"language({request.Language}) not support~");
            ...
            _logger.Info($"search {searchName} {request.Language} subtitle  result --->{searchResult} ");
            if (!string.IsNullOrWhiteSpace(searchResult))
            {
                var downloadUrl = await Api.SearchDownloadUrlAsync(client, subCatLanguage, searchResult, cancellationToken);
                _logger.Info($"search {searchName} {request.Language} subtitle  download url --->{downloadUrl} ");
